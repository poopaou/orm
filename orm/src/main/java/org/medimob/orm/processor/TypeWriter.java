...
class TypeWriter {
    ...
    public void writeType(TypeDefinition typeDefinition) throws IOException {
        ...
        JavaFileObject file = filer.createSourceFile(typeSimpleName + EntityProcessor.CLASS_MODEL_SUFFIX);
        ...
        // Type begin
        writer.emitEmptyLine();
        writer.beginType(typeDefinition.getTypeSimpleName() + "$$Dao", "class", EnumSet.of(PUBLIC, FINAL),
        ...
        writer.emitStatement("super(%s, %s, %s, %s, %s, %s)", TABLE_NAME_FIELD, ID_COLUMN_FIELD, VERSION_COLUMN_FIELD,
    ...
    private void readCursorForFields(JavaWriter writer, TypeDefinition definition, List<String> queryCols) throws IOException {
        writer.beginMethod(definition.getTypeSimpleName(), "readEntity", EnumSet.of(PUBLIC, FINAL), "Cursor", "cursor");
        writer.emitStatement("final %s entity = new %s()", definition.getTypeSimpleName(), definition.getTypeSimpleName());
        ...
        for(PropertyDefinition propertyDefinition : definition.getColumns()){
            ...
            switch (propertyDefinition.getPropertyType()){
                case BOOLEAN:
                    writer.emitStatement("entity.%s = %s.getBoolean(cursor, %s)", field, UTIL_CLASS_NAME, index);
                    break;
                case BYTE:
                    writer.emitStatement("entity.%s = %s.getByte(cursor, %s)", field, UTIL_CLASS_NAME, index);
                    break;
                case BYTE_ARRAY:
                    writer.emitStatement("entity.%s = %s.getByteArray(cursor, %s)", field, UTIL_CLASS_NAME, index);
                    break;
                case CHARACTER:
                ...
                case DATE_LONG:
                    ...
                    break;
                case DATE_STRING:
                    writer.emitStatement("entity.%s = %s.getDate(cursor, %s, %s)", field, UTIL_CLASS_NAME, index, quoteString(propertyDefinition.getDateFormat()));
                    break;
                case DOUBLE:
                    writer.emitStatement("entity.%s = %s.getDouble(cursor, %s)", field, UTIL_CLASS_NAME, index);
                    break;
                case FLOAT:
                    writer.emitStatement("entity.%s = %s.getFloat(cursor, %s)", field, UTIL_CLASS_NAME, index);
                    break;
                case INTEGER:
                ...
                case LONG:
                    ...
                    break;
                case SHORT:
                    writer.emitStatement("entity.%s = %s.getShort(cursor, %s)", field, UTIL_CLASS_NAME, index);
                    break;
                case STRING:
                    writer.emitStatement("entity.%s = %s.getString(cursor, %s)", field, UTIL_CLASS_NAME, index);
                    break;
            }
        }
        ...
        if (version != null) {
            writer.emitStatement("entity.%s = %s.getLong(cursor, %s)", idColumn.getFieldName(), UTIL_CLASS_NAME,
                    queryCols.indexOf(idColumn.getColumnName()));
        }
    ...
    private void createBindMethod(String methodName, JavaWriter writer, TypeDefinition typeDefinition, List<String> columnNames) throws IOException {
        writer.beginMethod("void", methodName, EnumSet.of(PUBLIC, FINAL), "SQLiteStatement", "statement",
        ...
        for (PropertyDefinition definition : typeDefinition.getColumns()){
            ...
            writer.emitStatement("%s.bind(statement, entity.%s, %s)", UTIL_CLASS_NAME, definition.getFieldName(), index);
        }
        writer.endMethod(); // END
    }


    private String formatStringArray(List<String> columns){
   ...
   /* private String formatSqlStatement(TypeDefinition typeDefinition){
    ...
    private void quoteString(StringBuilder builder, String string){
